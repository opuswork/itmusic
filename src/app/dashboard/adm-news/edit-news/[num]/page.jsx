...
import styles from '@/app/dashboard/adm-news/add-news/page.module.css';
...
export default function EditAdmNewsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/concerts/${num}`);
        if (cancelled) return;
        ...
        if (!cancelled) setError(err.response?.data?.message || '공연소식 정보를 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/dashboard/adm-news');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>공연소식 수정</h1>
            <Link href="/dashboard/adm-news" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-news-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-news" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
