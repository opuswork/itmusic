...
import styles from '@/app/dashboard/adm-notice/add-notice/page.module.css';
...
export default function EditAdmNoticePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/notices/${num}`);
        if (cancelled) return;
        if (res.data?.success && res.data?.data) {
          ...
          setFileName1(n.file_name1 ?? '');
          setOriginalFileName(n.original_file_name ?? '');
        }
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || '공지사항 정보를 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/dashboard/adm-notice');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            ...
            <Link href="/dashboard/adm-notice" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-notice-content-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
            <label className={styles.label}>첨부 이미지</label>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-notice" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
