...
import styles from '@/app/dashboard/adm-teachers/add-teacher/page.module.css';
...
export default function EditAdmTeacherPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/teachers/${num}`);
        if (cancelled) return;
        if (res.data?.success && res.data?.data) {
          const t = res.data.data;
          setOrderNum(t.order_num != null ? Number(t.order_num) : 0);
          setName(t.name ?? '');
          setPosition(t.position ?? '');
          setProfile(t.profile ?? '');
          setFileName1(t.file_name1 ?? '');
          setOriginalFileName(t.original_file_name ?? '');
        }
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || '지도위원 정보를 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/dashboard/adm-teachers');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>지도위원 수정</h1>
            <Link href="/dashboard/adm-teachers" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-teacher-profile-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-teachers" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
