...
import styles from '@/app/dashboard/adm-study/add-study/page.module.css';
...
export default function EditAdmStudyPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/studies/${num}`);
        if (cancelled) return;
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>유학정보 게시물 수정</h1>
            <Link href="/dashboard/adm-study" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
