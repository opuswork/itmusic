...
import styles from '@/app/dashboard/adm-consultants/add-consultant/page.module.css';
...
export default function EditAdmConsultantPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/consultants/${num}`);
        if (cancelled) return;
        if (res.data?.success && res.data?.data) {
          ...
          setOriginalFileName(c.original_file_name ?? '');
        }
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || '상임고문 정보를 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await http.put(`/consultants/${num}`, {
      ...
      router.push('/dashboard/adm-consultants');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>상임고문 수정</h1>
            <Link href="/dashboard/adm-consultants" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-consultant-profile-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-consultants" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
