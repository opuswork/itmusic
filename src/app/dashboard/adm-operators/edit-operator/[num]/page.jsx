...
import styles from '@/app/dashboard/adm-operators/add-operator/page.module.css';
...
export default function EditAdmOperatorPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/operators/${num}`);
        if (cancelled) return;
        if (res.data?.success && res.data?.data) {
          const o = res.data.data;
          setOrderNum(o.order_num != null ? Number(o.order_num) : 0);
          setName(o.name ?? '');
          setPosition(o.position ?? '');
          setProfile(o.profile ?? '');
          setFileName1(o.file_name1 ?? '');
          setOriginalFileName(o.original_file_name ?? '');
        }
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || '운영위원 정보를 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/dashboard/adm-operators');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>운영위원 수정</h1>
            <Link href="/dashboard/adm-operators" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-operator-profile-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-operators" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
