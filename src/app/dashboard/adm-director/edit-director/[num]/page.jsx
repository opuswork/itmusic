...
import styles from '@/app/dashboard/adm-director/add-director/page.module.css';
...
export default function EditAdmDirectorPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await http.get(`/directors/${num}`);
        if (cancelled) return;
        if (res.data?.success && res.data?.data) {
          ...
          setText(d.text ?? '');
          setFileName1(d.file_name1 ?? '');
          setOriginalFileName(d.original_file_name ?? '');
        }
      } catch (err) {
        if (!cancelled) setError(err.response?.data?.message || '음악감독 정보를 불러오지 못했습니다.');
      } finally {
  ...
  useEffect(() => {
    ...
    if (initialTextSet.current) return;
    try {
      window.$(editorRef.current).summernote('code', plainTextLineBreaksToHtml(text ?? ''));
      initialTextSet.current = true;
    } catch (_) {}
  }, [scriptsReady, loading, text]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await http.put(`/directors/${num}`, {
        name: name.trim() || '',
        position: position.trim() || '',
        text: getEditorContent()?.trim() || null,
        file_name1: finalFile_name1,
        originalFileName: finalOriginalFileName,
      });
      router.push('/dashboard/adm-director');
      router.refresh();
    } catch (err) {
      ...
      <>
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderInner}>
            <h1 className={styles.pageTitle}>음악감독 수정</h1>
            <Link href="/dashboard/adm-director" className={styles.backLink}>목록으로</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            {!scriptsReady ? (
              ...
              <div ref={editorRef} id="summernote-director-text-edit" />
            )}
          </div>
          <div className={styles.formGroup}>
          ...
          <div className={styles.formActions}>
            ...
            <Link href="/dashboard/adm-director" className={styles.cancelButton}>
              취소
            </Link>
          </div>
        </form>
