...
export async function GET(request, { params }) {
  try {
    ...
    if (!teacher) {
      return Response.json({ success: false, message: '지도위원을 찾을 수 없습니다.' }, { status: 404 });
    }
    return Response.json({ success: true, data: serializeBigInt(teacher) });
  } catch (error) {
    console.error('Error in GET /api/teachers/[id]:', error);
    return Response.json(
      { success: false, message: '지도위원 조회 중 오류가 발생했습니다.', error: error.message },
      { status: 500 }
    );
  }
}
...
export async function PUT(request, { params }) {
  try {
    ...
    return Response.json(
      { success: false, message: '지도위원 수정 중 오류가 발생했습니다.', error: error.message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.teachers.delete({ where: { num: id } });
    ...
    return Response.json(
      { success: false, message: '지도위원 삭제 중 오류가 발생했습니다.', error: error.message },
      { status: 500 }
    );
  }
}
